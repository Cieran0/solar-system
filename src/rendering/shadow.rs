...
// Omnidirectional shadow mapping based on learnopengl.com shadow tutorial (modified)
pub struct ShadowRenderer {
    pub depth_cubemap: GLuint,
    pub depth_fbo: GLuint,
    pub shader: GLuint, // depth-pass program
    light_pos: Vec3,
    pub shadow_far: f32,
    size: i32,
}
impl ShadowRenderer {
    // Initializes the shadow renderer with a cubemap framebuffer, depth texture, and dedicated shaders.
    ...
    // Computes the six view-projection matrices for rendering each face of the shadow cubemap.
    ...
    // Renders all non-emissive objects into the shadow cubemap for omnidirectional shadow mapping.
...
// Cleans up shadow renderer GPU resources when dropped.
